...
public class IndexVerifier {
    ...
    public void createIndexesIfNotExist() {
        // T 26/08/2020 There is no index creation via annotation support
        ...
        INDEX_TO_INDEX_OPTIONS.entrySet()
                ...
                .forEach(entry -> {
                    ...
                    var index = Indexes.ascending(entry.getKey());

                    // Use ifPresentOrElse for a cleaner functional approach
                    entry.getValue().ifPresentOrElse(
                            options -> mongoCollection.createIndex(index, options),
