...
public class BalanceChangeChecker {
    ...
    private Map.Entry<UserDb, Response> updateUser(Map.Entry<UserDb, SodexoResponse> tuple) {
        ...
        var messageText = newOperations
                .stream()
                .map(h -> String.format(
                        "%s %.2f rub from %s", h.getAmount() > 0 ? "Deposit" : "Withdrawal", Math.abs(h.getAmount()), h.getLocationName().get(0)
                ))
                .collect(Collectors.joining("\n", "", String.format("\nCurrent balance %.2f rub", sodexoResponse.getData().getBalance().getAvailableAmount())));
