...
public class DefaultHandler implements InputTextHandler {
    ...
    public Uni<Response> handle(Long chatId, String text) {
        ...
        if (byChatId != null && byChatId.getCard() != null) {
            return Uni.createFrom().item(Response.withReplyButtons("Unknown request :(", replyButtonsProvider.provideMenuButtons()));
        } else {
            if (text.matches("[0-9 ]+")) {
                        ...
                        .flatMap(sodexoResponse -> {
                            if (sodexoResponse.getStatus().equals(Constants.OK_STATUS)) {
                                ...
                                return userRepository.persistOrUpdate(userDb)
                                        .map(v -> Response.withReplyButtons("I saved card " + noSpacesCard, replyButtonsProvider.provideMenuButtons()));
                            } else if (sodexoResponse.getStatus().equals(Constants.CARD_IS_NOT_ACTIVE_STATUS)) {
                                return Uni
                                        .createFrom()
                                        .item(Response.fromText("Cards " + text + " is outdated or inactive for other reasons, please try again"));
                            } else {
                                return Uni
                                        .createFrom()
                                        .item(Response.fromText("Cards " + text + " does not exist, please try again"));
                            }
                        });
            } else {
                ...
                return Uni
                        .createFrom()
                        .item(Response.fromText("Invalid card format"));
            }
        }
    }
